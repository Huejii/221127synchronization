...
int temp; // 데이터를 옮기기 위한 임시 저장소
    ...
    pthread_mutex_lock (&mutex);
    while(teamA_buffer->head - teamA_buffer->tail == 1)
    {
        pthread_cond_wait (&A_prod, &mutex);
    }
    //producer
